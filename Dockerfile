...
RUN npm run build
EXPOSE $PORT
ENTRYPOINT node server.js --path ./dist --port $PORT --env "{\"EDDI_API_URL\":\"$EDDI_API_URL\", \"AUTH_METHOD\":\"$AUTH_METHOD\", \"AUTH_URL\":\"$AUTH_URL\", \"AUTH_REALM\":\"$AUTH_REALM\", \"AUTH_CLIENT_ID\":\"$AUTH_CLIENT_ID\", \"READ_ONLY\":\"$READ_ONLY\"}"
