 ...
 *   it; restating a value that differs from the pin would be refused with 409.
...
export function requestFromDraft(
  ...
  const field = <T,>(setting: ConnectionSetting<T>, value: T): T | null => {
    if (setting.source === "PINNED") return null;
    if (setting.source === "DEFAULT" && sameValue(setting.value, value)) return null;
    return value;
  };
